...
def _pack_one(out, v):
    if v is None:
        ...
        # Pack as signed 64-bit if fits, otherwise marshal
        if -(2**63) <= v <= 2**63 - 1:
...
def pack_co(co):
    ...
    try:
        ...
        raw_code = co.co_code if hasattr(co, 'co_code') else bytes(co.co_code)
        swapped, rev = _swap_ops(raw_code, set(starts))
    except Exception:
        starts = []
        raw_code = co.co_code if hasattr(co, 'co_code') else b''
        swapped = raw_code
        rev = {}
